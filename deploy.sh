...
deploy_token() {
    ...
    # Prompt for RPC URL and block explorer
    read -p "Enter the RPC URL for the target network: " RPC_URL
    read -p "Enter the base URL of the block explorer (e.g., https://sepolia.uniscan.xyz/): " BLOCK_EXPLORER
...
EOF
    ...
    for PRIVATE_KEY in "${KEY_ARRAY[@]}"; do
        ...
        show_message "Token deployed successfully with private key $PRIVATE_KEY, contract address: $BLOCK_EXPLORER$CONTRACT_ADDRESS" "success"
    done
...
main_menu() {
    while true; do
        ...
        echo -e "${HEADER_COLOR}${BOLD}================ Contract Deployment =================${RESET_COLOR}"
